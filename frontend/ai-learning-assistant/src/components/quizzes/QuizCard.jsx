...
const QuizCard = ({ quiz, onDelete }) => {
  // completedAt is the most reliable signal — it's null by default and
  // only set by the backend when the quiz is actually submitted.
  // This handles score=0 correctly too.
  const isAttempted = !!quiz?.completedAt;
        ...
        <div className="inline-flex items-center gap-1.5 py-1 rounded-lg text-xs font-semibold">
          {isAttempted ? (
            <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1">
              <Award className="w-3 h-3.5 text-emerald-600" strokeWidth={2.5} />
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1">
              <Award className="w-3 h-3.5 text-slate-400" strokeWidth={2.5} />
